...
are attended together, the thread inherits everything that context's subagent
...
does not record: an assigned event stays `PENDING` until the subagent reports.
...
2. Claim every pending event whose stream routes to the live context:
   ...
   the context's subagent as assignee. Everything pending is taken at once, so
...
3. Append the events as turns plus one brief, `live.md.j2`: you are in this
   conversation as it happens; answer what wants answering, and say nothing
   if nothing does. The brief is a turn rather than a different system
   prompt, so the cached prefix survives the change of mode.
4. Call the context's subagent with one tool, `yield_focus(reason)`. Append
...
the point is to answer in seconds, and the context is the same one a
higher-effort subagent will continue later.
...
refused for a job stream. `pull(stream_id, reason)` routes another stream into
...
a subagent replying elsewhere. The control plane and triage never share a
...
`focus` on a job stream is refused; `pull` of a stream with its own context is
...
thread should ever run tools of its own; here it is the same single call as
every other subagent, and describes what it would send.
