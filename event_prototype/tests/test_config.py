...
def test_urgent_priority_and_heartbeat_are_read_from_the_file(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
    path.write_text('urgent_priority = "realtime"\nheartbeat_seconds = 30\n')

    config = Config.load(path)

    assert config.urgent_priority is Priority.REALTIME
    assert config.heartbeat_seconds == 30.0


def test_an_unknown_priority_name_is_rejected(tmp_path: Path) -> None:
    path = tmp_path / "config.toml"
    path.write_text('urgent_priority = "urgent"\n')
