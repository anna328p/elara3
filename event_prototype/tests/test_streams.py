...
async def test_events_in_a_room_join_one_stream_row(queue: EventQueue) -> None:
    first = await queue.submit(message(sender="mira", conversation="#workshop"), Priority.BACKGROUND)
    second = await queue.submit(message(sender="hal", conversation="#workshop"), Priority.BACKGROUND)
    elsewhere = await queue.submit(message(sender="quill", conversation="#general"), Priority.BACKGROUND)
...
async def test_last_activity_is_the_newest_event_not_the_last_submitted(
    ...
    await queue.submit(recent, Priority.BACKGROUND)
    await queue.submit(old, Priority.BACKGROUND)
...
async def test_a_completed_event_leaves_the_stream_but_not_the_count(
    ...
    first = await queue.submit(message(sender="mira", conversation="#workshop"), Priority.BACKGROUND)
    await queue.submit(message(sender="hal", conversation="#workshop"), Priority.BACKGROUND)
...
async def test_the_stream_rides_along_and_survives_the_session(
    ...
    """
    await queue.submit(message(sender="mira", conversation="#workshop"), Priority.NUDGE)
    deferred = await queue.submit(message(sender="hal", conversation="#workshop"), Priority.BACKGROUND)
...
async def test_listing_every_stream_is_one_query(queue: EventQueue) -> None:
    for sender, where in (("mira", "#workshop"), ("hal", "#workshop"), ("q", "#general")):
        await queue.submit(message(sender=sender, conversation=where), Priority.BACKGROUND)
...
async def test_the_stream_reaches_the_prompt_and_the_backlog_line(
    ...
    pending = await queue.submit(message(sender="mira", conversation="#workshop"), Priority.NUDGE)
    deferred = await queue.submit(message(sender="quill", conversation="#general"), Priority.BACKGROUND)
